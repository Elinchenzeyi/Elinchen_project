...
results. 
Specifcially, SHAP provides the valuable and consistent explanations with 
linear regression's global feature importance, but what's more, I think
SHAP even also explores the non-linear effects the linear model misses to 
...
I think SHAP shows strong local consistency: 

Like for similar observations, SHAP tends to attribute similar feature 
contributions to the prediction. In using k-nearest neighbors or clustered 
groups, SHAP values for features like lstat and ptratio suggests that the 
Random Forest model behaves consistently within the local regions of the 
input space. 
...
However, random forest captures the non-linear and iteraction effects. So
it could be the fact that nox's signal overlaps with other features or nox 
affects the price only as extreme values. 
